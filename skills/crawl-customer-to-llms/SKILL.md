...
effort: high
description: >-
  Walk a whole customer engagement folder on the <REDACTED-TS-PREMIUM-SERVICES> shared drive AND
...
`/Users/mitch.hudson/Library/CloudStorage/GoogleDrive-<REDACTED-MITCH-HUDSON-MONGODB-COM>/Shared drives/<REDACTED-TS-PREMIUM-SERVICES> - <REDACTED-TAM-NTSE>/Engagements/`
...
- Resolve the argument against the engagement root's directory listing, case-insensitively
  and on substrings. Known aliases: `<REDACTED-GS>` → `<REDACTED-GS>` (<REDACTED-GOLDMAN-SACHS>), `<REDACTED-JPMC>`/`JPM`/`<REDACTED-JPMORGAN>` →
  `<REDACTED-JPMORGAN-CHASE>`, `BoA` → `<REDACTED-BANK-OF-AMERICA>`, `DB` → `<REDACTED-DEUTSCHE-BANK>`, `<REDACTED-CUST-I>` → `Internal
  Revenue Service`. Several folders can match one account (`<REDACTED-CUST-B>` and `<REDACTED-CUST-B> (1)`;
  `<REDACTED-CUST-D>` and `<REDACTED-CUST-D> - Streaming Applications`) — treat those as **one customer with
...
- Establish the customer's public-name variants for Glean (`<REDACTED-GS>` → "<REDACTED-GOLDMAN-SACHS>",
  "Goldman", "<REDACTED-GS>"); the folder name alone is a poor query term.
...
- **Taxonomy detection.** Some accounts (<REDACTED-GS>, <REDACTED-JPMORGAN-CHASE>) carry the numbered doc-store
  ...
  absent (Apple, <REDACTED-CUST-C> and most accounts are freeform), fall back to name and content
...
- **Oversize results.** A large doc (the <REDACTED-GS> context file is ~100k characters) exceeds the
...
| `when` | when a future agent would open it ("prepping the weekly", "case <CASE-NUMBER> recurs") | derived |
...
8. **Boundary check** — grep the emitted files for the other customers' names from the
   ...
   grep does not: short folder names (`<REDACTED-GS>` is two characters — a length threshold silently
   skips it, and it appears 7 times in the <REDACTED-JPMC> pack), and folders whose names are also
   MongoDB products or common words (`Atlas`, `Apple`, `<REDACTED-CUST-F>`, `<REDACTED-CUST-D>`), which otherwise
...
- **Multi-root account** (`<REDACTED-CUST-B>` + `<REDACTED-CUST-B> (1)`) → crawl all roots, label every card with
