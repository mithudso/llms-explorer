...
effort: high
description: >-
  Walk a whole customer engagement folder on the [redacted] shared drive AND
...
`/Users/mitch.hudson/Library/CloudStorage/GoogleDrive-[redacted]/Shared drives/[redacted] - [redacted]/Engagements/`
...
- Resolve the argument against the engagement root's directory listing, case-insensitively
  and on substrings. Known aliases: `[redacted]` → `[redacted]` ([redacted]), `[redacted]`/`JPM`/`[redacted]` →
  `[redacted]`, `BoA` → `[redacted]`, `DB` → `[redacted]`, `[redacted]` → `Internal
  Revenue Service`. Several folders can match one account (`[redacted]` and `[redacted] (1)`;
  `[redacted]` and `[redacted] - Streaming Applications`) — treat those as **one customer with
...
- Establish the customer's public-name variants for Glean (`[redacted]` → "[redacted]",
  "Goldman", "[redacted]"); the folder name alone is a poor query term.
...
- **Taxonomy detection.** Some accounts ([redacted], [redacted]) carry the numbered doc-store
  ...
  absent (Apple, [redacted] and most accounts are freeform), fall back to name and content
...
- **Oversize results.** A large doc (the [redacted] context file is ~100k characters) exceeds the
...
8. **Boundary check** — grep the emitted files for the other customers' names from the
   ...
   grep does not: short folder names (`[redacted]` is two characters — a length threshold silently
   skips it, and it appears 7 times in the [redacted] pack), and folders whose names are also
   MongoDB products or common words (`Atlas`, `Apple`, `[redacted]`, `[redacted]`), which otherwise
...
- `tam-account-reports` / `[redacted-skill]` / `gs-account-pipeline`: consume the pack;
...
- **Multi-root account** (`[redacted]` + `[redacted] (1)`) → crawl all roots, label every card with
