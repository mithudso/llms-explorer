...
def render(subject: str, entries: list[dict], generated: str) -> str:
    n_def = sum(1 for e in entries if e["definition"])
    ...
    for e in entries:
        ...
        out.append(line)
    undefined = [e for e in entries if not e["definition"]]
    if undefined:
        out += ["", "## Named, not yet defined", "",
                "Terms the pool names (≥ 2 uses) without a sentence that defines them — "
                "research gaps."]
        out += [f"- {e['term']}" + (f" · aka: {', '.join(e['aka'])}" if e["aka"] else "")
                for e in undefined]
    return "\n".join(out).rstrip() + "\n"
