---
title: "Measuring a Thunderbolt eGPU: bandwidth, latency and inference benchmarks"
description: "How to measure a Thunderbolt eGPU on Linux instead of trusting estimates — host-to-device bandwidth with nvbandwidth and bandwidthTest, inference benchmarks with llama-bench and Ollama, measuring mode"
---

# Measuring a Thunderbolt eGPU on Linux - bandwidth, inference benchmarks, load time, methodology

How to measure a Thunderbolt eGPU on Linux instead of trusting estimates — host-to-device bandwidth with nvbandwidth and bandwidthTest, inference benchmarks with llama-bench and Ollama, measuring model load time separately from generation, methodology and pitfalls, which workloads are link-bound, and a results template for catching regressions after a kernel, driver, cable or BIOS change.
