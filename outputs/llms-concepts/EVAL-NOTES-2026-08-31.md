...
- **Pack indexing.** All 8 concept packs have vector + FTS5 layers as `concept__<slug>`, each at exact row parity with its `units.jsonl` (121–2,013 rows). Retrieval spot-checked: keyword `covered query` on the indexing pack, vector "how does the cardiac cycle work" on the heart pack → 0.811 on the cardiac-cycle definition.
- **Docset facts layers.** The 10 layers the re-extraction made stale were re-indexed from their new `all_units.jsonl`; every one grew (e.g. code.claude.com 13,976 → 14,331; docs.claude.com 13,382 → 13,665; mongodb 1,349 → 1,769 at exact parity). Where the indexed count sits a few rows under the file count, the gap is **exactly** the indexer's 20-character minimum (`load_units`, table fragments like `Connectors: None`) — verified per docset (glean 6, markedapp 8, paypal.ai 20), nothing dropped silently.
