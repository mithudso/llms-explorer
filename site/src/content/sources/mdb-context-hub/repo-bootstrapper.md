---
title: "Repo Bootstrapper"
description: "Generates and maintains the full mdb-tam file standard for any repository."
---
...
- Create an ideal repository from scratch using the full mdb-tam standard (includes code quality optimization and LLM doc generation)
- Upgrade an existing repository toward that same ideal state (runs cdo and crawl-repo-to-llms automatically)
...
- Keep docs/high_signal_file_index.json and docs/codebase-overview.md current after large changes (regenerated by crawl-repo-to-llms)
- Optimize code quality and generate LLM-friendly documentation as part of standard repo modernization
...
| "clean up", "commit all", "get repo current and clean" | `cleanup_and_commit` |
| "register with hub", "add to global-ai-hub", "sync with project manager" | `register_with_hub` |
...
**Bootstrap prompt fallback:** if `[redacted]/mdb-tam/docs/repo-bootstrap-prompt.md` cannot be loaded, stop and ask for the path to the local mdb-tam checkout. Do not proceed with `run_bootstrap` or `create_ideal_repo` without it.
...
- This skill's canonical bootstrap prompt lives in `[redacted]/mdb-tam` at `docs/repo-bootstrap-prompt.md`.
...
| `docs/tool-inventory.json` | Tool/route inventory. In `[redacted]/mdb-tam` this is the server's HTTP routes with human-curated descriptions — a faithful drift check is a route-name set diff and requires a `createApp()` entry-point refactor (pending); until then treat as curated, verify counts manually |
...
3. **Code quality pass** — run `code-deep-optimizer` (cdo) to audit source code for bugs, efficiency, and maintainability; apply all Medium+ findings automatically.
4. **LLM documentation pass** — run `crawl-repo-to-llms` to generate machine-readable LLM-friendly documentation indexes (`docs/high_signal_file_index.json`, llms.txt metadata, semantic embeddings).
5. **Convergence loop** — re-run the drift pass after each change set. Stop when the audit returns zero medium-or-higher findings. Hard cap: 3 iterations. After 3, surface remaining findings as a TODO list and stop.
6. **Repo hygiene pass** — commit all changes, stage untracked files, clean up dangling work, and leave repo in a clean state with no shadow branches or uncommitted changes.
7. **Hub registration pass** — register or update the project in `~/.global-ai-hub` project registry; sync metadata (paths, tags, languages, description) if already registered.
8. **No-invention rule** — never fabricate a command, path, env var, or external service. Surface unknowns as **TODO with where-to-find**.
...
| `cleanup_and_commit` | "clean up this repo", "commit all changes", "finalize the repo state", "get the repo current and clean" |
| `register_with_hub` | "register this repo with the hub", "add to global-ai-hub", "update hub registry", "sync with project manager" |
...
4. Run `code-deep-optimizer` (cdo) to audit and optimize source code from the start
5. Run `crawl-repo-to-llms` to generate LLM-friendly documentation and semantic indexes for the new repo
6. **Repo hygiene & commit** — stage all new files, commit bootstrap changes, ensure clean working tree
7. **Hub registration** — register project in `~/.global-ai-hub` registry with metadata (path, languages, tags, description)
...
4. Run `code-deep-optimizer` (cdo) to audit and fix source code — auto-applies all Medium+ findings
5. Run `crawl-repo-to-llms` to generate LLM-friendly documentation indexes and semantic metadata
6. Re-run audit after code and doc passes to validate no regressions
7. **Repo hygiene & commit** — stage all untracked files, commit all changes (with descriptive message), clean up any shadow branches or dangling work, ensure clean `git status`
8. **Hub registration** — register or update project in `~/.global-ai-hub` registry; sync repo metadata (paths, tags, languages, description)
9. Prefer clean rewrites over incremental patching when a file has duplication, drift, or low-signal structure
...
(loaded from `[redacted]/mdb-tam/docs/repo-bootstrap-prompt.md`).
...
1. Get the repo current — pull latest from remote if applicable
2. Stage all untracked files in the repo
3. Commit all uncommitted changes with a descriptive message (format: "repo: modernization & cleanup" or similar)
4. Identify and clean up any shadow branches or dangling work (stashed changes, detached HEAD, leftover branches)
5. Verify `git status` returns clean — no uncommitted changes, no untracked files
6. Report final state: commit shas, branches cleaned, files committed

### `register_with_hub(path)` → hub registry entry

1. Check if project exists in `~/.global-ai-hub/registry.json` (or equivalent project manager)
2. If NOT registered: create new entry with metadata (absolute path, project name, languages, primary tags from package.json/docs, description from README)
3. If already registered: update metadata (sync paths, update tags, refresh description, update last-modified timestamp)
4. Validate entry against hub schema
5. Write/update registry and report final state: entry ID, metadata synced, timestamp

### `run_bootstrap(repo_path)` → full task prompt

1. Load `docs/repo-bootstrap-prompt.md` from `[redacted]/mdb-tam` (not from the target repo)
...
The full bootstrap prompt lives at `docs/repo-bootstrap-prompt.md` in `[redacted]/mdb-tam`. Load it when the user asks to initialize, modernize, or turn a repository into an ideal repo. Do not hard-code local filesystem paths — resolve the file at runtime from the `[redacted]/mdb-tam` checkout on the current machine.
