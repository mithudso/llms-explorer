# Case Study — Gamifying Incident-Response Training with the [redacted] Rate-Limiting Fire Drill
...
- **Solution:** A gamified, fully-isolated fire drill (Scenario 4 — *[redacted] rate limiting / Atlas API throttle*) run under the JIMP role structure and scored on a coordination-weighted rubric.  
...
### §2.2 The [redacted] scenario

Scenario 4 (`scenario-4.json`) — *"[redacted] rate limiting — Atlas API throttle"* (severity 2, playbook §4.4) — was chosen as the basis. It is realistic, multi-step, and ambiguous: a simulated customer (drill persona "Maya S.," calm and evidence-seeking) reports that Atlas API calls are being throttled. The scenario carries:
...
- **Coverage:** expand beyond the five current scenarios; vary surface features of the [redacted] rate-limiting scenario so teams build transferable fault scripts rather than memorizing one path.  
...
| [redacted] scenario | `mcp-server/data/firedrill/scenarios/scenario-4.json` | Rate-limiting scenario, rubric, complications, persona |
