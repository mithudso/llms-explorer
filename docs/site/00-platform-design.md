...
|---|---|---|---|
| 1 | 03, 05, 11, 12 (intro + grammar), 14 (decision table + copy-only recipes), 04 launch posts, 01 lint in CI | published content pages with `.md` twins, the site's own `/llms.txt` + facts + vocabulary, `overrides` hook in `export_llms`, `llms_lint.py --kind vocabulary`, snapshot → `snapshot` branch with CI promotion to `main` | `llms_lint.py check` over the site's own llms files exits 0 in CI on every push; every page has a twin; the 8 launch posts render with numbers from `outputs/` |
