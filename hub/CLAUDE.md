...
- llms-full.txt mirror: `.venv/bin/python scripts/llms_full_catalog.py {compile [--seed URL] [--offline FILE] | add-seed URL [--name N] [--category C] | download [--jobs N] [--max-bytes N] [--only S] [--retry-failed|--refresh] | list [--status ok|rejected|failed|missing|all] [--query S] [--min-pages N] [--json] | delete KEY | export-mirror KEY OUT.md}` — catalog of every site known to publish `llms-full.txt` (llms-txt-hub, llmstxt.site, directory.llmstxt.cloud, own docslist probe) + downloads into `llms-full/files/` (gitignored); served read-only by MCP `hub_llms_full_list` (default `min_pages=1` hides 0-page blobs) / `hub_llms_full_read`; refreshed weekly by launchd `com.global-ai-hub.llms-full-refresh` (`scripts/launchd/llms-full-refresh.sh`)
- llms-full private discovery library: `.venv/bin/python scripts/llms_full_library.py {add URL | check | incorporate | stale [--max-age-days N] | requeue-stale [--max-age-days N] | list [--status pending|downloaded|rejected|incorporated|all] [--json]}` — a queue of OTHER llms-full.txt aggregator/directory pages (beyond the three `compile` already crawls) to check, sweep for site URLs, and incorporate into the public catalog via `add-seed`; the aggregator page itself is archived under `llms-full/private/` (gitignored, never synced by `refresh_snapshot.sh`) — only newly-discovered SITE urls ever reach the public catalog/mirror, never an aggregator's own compiled content
...
- Docset rollout: `.venv/bin/python scripts/docset_rollout.py {probe | apply [--group llms-full|llms|crawl|all] [--dry-run] | cleanup [--dry-run]}` — which hosts publish `llms.txt`, move a trafilatura mirror aside + reset its queue item, delete distill-era artifacts once a fact layer exists; `cleanup` ends with the llms lint over every `<stem>.llms/` export and exits 1 on any High (`--no-lint` skips)
- llms-explorer snapshot (github.com/mithudso/llms-explorer, `~/dev/llms-explorer`): the self-supported copy of the llms work — refreshed by that repo's `scripts/refresh_snapshot.sh`, run daily 04:30 by launchd `com.llms-explorer.snapshot-refresh` (`scripts/launchd/llms-explorer-refresh.sh`) and after every pipeline queue drain
...
- `scripts/embed_core.py`: weighted multi-host Ollama pool — `HUB_OLLAMA_URLS`, default linux GPU box (192.0.2.75)=4 > M3 Mac (192.0.2.113)=3 > this machine/M5 localhost=1; `HUB_EMBED_MODEL` default `mxbai-embed-large`.
...
- `mcp-server/hub_mcp_server.py`: 18 `hub_*` tools over the above (incl. the `llms-full/` mirror via `hub_llms_full_list`/`_read`) (incl. `hub_ask` federated answer, `hub_search_symbols`, `hub_route`) + distillers kickoff + llm-memory-pyramid search. Localhost trust model; never expose off-box without adding auth.
