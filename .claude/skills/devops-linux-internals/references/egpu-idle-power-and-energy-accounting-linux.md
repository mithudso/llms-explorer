...
name: egpu-idle-power-and-energy-accounting-linux
title: Idle Power and Energy Accounting for an Always-On NVIDIA eGPU LLM Box (Linux)
description: TRIGGER: working out where an always-on Thunderbolt NVIDIA eGPU LLM box spends idle watts, measuring wall/GPU/host power honestly, choosing levers (power limit, clock lock, keep-alive, enclosure off), converting watts to kWh and cost, exporting power metrics and alerting on idle creep. SKIP: PSU/12V-2x6/thermals (egpu-power-enclosure-and-thermals-linux.md), ASPM/D3cold/runtime PM internals (pcie-power-management-aer-dpc-egpu-linux.md), sleep states (egpu-suspend-resume-and-sleep-states-linux.md), throughput benchmarking.
