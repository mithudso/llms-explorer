---
title: "Reproducible eGPU bring-up and configuration drift detection"
description: "Keeping the whole working Thunderbolt eGPU setup restorable: an inventory of every piece of state involved, capture with an idempotent installer and a checksum manifest, a read-only drift verifier for"
---

# Reproducible Thunderbolt NVIDIA eGPU Bring-up, Drift Detection and Restore on Ubuntu

Keeping the whole working Thunderbolt eGPU setup restorable: an inventory of every piece of state involved, capture with an idempotent installer and a checksum manifest, a read-only drift verifier for after kernel, driver or release upgrades, the restore order after a reinstall, and rollback and pinning.
