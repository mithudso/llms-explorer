...
class SkillParseError(ValueError):
    """The file exists but is not a skill spec (no frontmatter, or unreadable)."""
...
def _split_frontmatter(text: str) -> tuple[str, str]:
    ...
    skill to the model stripped of the half that routes it.
    """
...
class Skill:
    ...
    def read_references(self) -> dict[str, str]:
        return {p.name: p.read_text(encoding="utf-8") for p in self.reference_files()}
...
def _repo_skills_dirs() -> list[Path]:
    ...
    Same walk-upward rule `tree.default_data_path()` uses, so a checkout in an
    unusual place behaves the same for skills as it does for the concept tree.
    """
    found: list[Path] = []
...
def load_skill(name: str, *, search_paths: Iterable[Path] | None = None) -> Skill:
    ...
    the call site instead of arriving as an empty prompt at the API.
    """
...
def load_skill_file(path: str | Path, *, name: str | None = None) -> Skill:
    ...
    frontmatter_src, body = _split_frontmatter(p.read_text(encoding="utf-8"))
...
def available_skills(*, search_paths: Iterable[Path] | None = None) -> list[str]:
    ...
    for directory in paths:
        ...
        for child in sorted(directory.iterdir()):
            if (child / "SKILL.md").is_file() and child.name not in names:
