...
**Scope note.** This catalog explains the skills across five families — **MongoDB, psychology, writing, troubleshooting, and expertise** — that together support automated and human-assisted MongoDB/Atlas case resolution. "Expertise" is covered in **both senses**: the domain-authority skills (the deep `*-expert` hubs) *and* the expertise-engineering machinery that captures, teaches, measures, and optimizes expertise. A later section explains the optimization-pipeline machinery that connects them — it's also the subject of the companion whitepaper.
...
| `atlas-diagnostics-expert` | **Live diagnostics, performance, monitoring, and capacity** hub: `[redacted]`, FTDC/log tooling, Performance Advisor, Atlas alert conditions, benchmarking (YCSB), observability (Prometheus/Datadog/Grafana), capacity planning. Also owns the `@mdb-tam/atlas-diagnostics` Chrome-extension package. | The triage surface — chosen first when a *live* case has a performance or health symptom. |
...
| `[redacted]` | **[redacted] GitHub repo intelligence**: repo prioritization, symptom→repo mapping, install/run guidance, the diagnostic-tool catalog (FTDC, explain plans, Jira enrichment). | Maps a diagnostic scenario to the right internal tool or repo. |
...
| `ddo` *(Document Deep Optimizer)* | The apply-in-place driver over `document-critique`: runs the multipass critique and **applies every medium-or-higher fix**, looping to convergence (max 3 iterations, raised to 5 if Medium+ findings dropped ≥50% in the prior iteration). Fast paths: `--voice-only`, `--read-only`, `--annotate`. | One command to take a draft to convergence. |
...
| `[redacted]` *(cross-listed from §1)* | The diagnostic-tool catalog and symptom→repo mapping. | Points the diagnosis at the right instrument. |
...
- `design-deep-optimizer` — UI/visual designs (11 passes); not part of this case-resolution pipeline, listed for sibling-family completeness  
...
*Source:* the internal `[redacted]` evaluation writeup for this panel.
...
1. **Troubleshooting** chooses the diagnostic surface and gathers evidence (`atlas-diagnostics-expert`, `[redacted]`, `software-engineering-patterns`).  
