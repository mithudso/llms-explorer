---
title: "LLM Inference Optimization and Serving"
description: "PROVENANCE: This reference is part of the ai-agent-engineering hub."
---
...
Owns the LLM **serving / inference-runtime** layer. Boundaries:
  ...
  - Multi-LoRA adapter DECISIONS (merge vs swap, `add_weighted_adapter` combination math, which adapters to serve) → `llm-fine-tuning-peft`. Here we only cover the serving-runtime side of hosting many adapters (S-LoRA/Punica-style paging and batched kernels).
  ...
  - Managed AWS inference (Bedrock/SageMaker endpoints) → aws-ai-ml.
  - Production observability of a running endpoint (tracing, eval, drift) → llm-observability reference.
  - LLM model landscape / selection → llm-models reference.
-->
