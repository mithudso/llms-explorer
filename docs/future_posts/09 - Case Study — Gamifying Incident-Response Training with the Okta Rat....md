...
Scenario 4 (`scenario-4.json`) — *"<REDACTED-OKTA> rate limiting — Atlas API throttle"* (severity 2, per the IR playbook) — was chosen as the basis. It is realistic, multi-step, and ambiguous: a simulated customer (drill persona "Maya S.," calm and evidence-seeking) reports that Atlas API calls are being throttled. The scenario carries:
