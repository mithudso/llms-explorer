---
title: "[redacted] Repo Intelligence"
description: "- Identify which [redacted] repo or script is most relevant to a case-tracker or troubleshooting scenario"
---

# [redacted] Repos — Case Tracker and Troubleshooting Extension Context

## When to use this skill

- Identify which [redacted] repo or script is most relevant to a case-tracker or troubleshooting scenario
...
- General Atlas troubleshooting with no [redacted] repo angle — use `atlas-diagnostics-expert`
...
- **Run:** `npm run dev`
- **Notes:** UI-focused; easier to mine for patterns than [redacted].

### [redacted] (SearchPlanIQ)
...
- **Run:** `./[redacted] <diagnostic.data path>`
...
| `[redacted]` | Large hunting ground for niche internal support utilities |
| `scripts-and-snippets` | Internal script catalog and discovery point |
| `[redacted]` | Atlas node-side support scripts |
...
|----------|--------------|-----------|
| Create or update a support case | [redacted], mdb-tam, [redacted] | [redacted] automation endpoints |
| Enrich a case with Jira/Salesforce/support-alert context | [redacted], [redacted], [redacted] | Jira/SFDC scrapers |
| Collect or analyze FTDC diagnostics | [redacted], [redacted], ts-tools-diagnostic-viewer | FTDC request API; then Alexandria CLI |
| Analyze Atlas Search or vector search explain plans | [redacted], ts-query-helper | SearchPlanIQ app and MCP server |
| Prioritize cases by support load or signal volume | [redacted], support-dashboard | Support volume tracker |
| Retrieve runbooks, knowledge, or engineering context | [redacted], [redacted], agent-skills | Confluence/Git/Backstage via MCP |
...
| `scripts/getPickListValues.ts` | [redacted] | Salesforce picklist sync utility |
| Support API endpoints | [redacted] | Programmatic case automation and auth surface |
| `jira_scraper.py` | [redacted] | Jira ingestion for support DB synchronization |
| `support_demand_tracker.py` | [redacted] | Aggregates support demand signals |
| `searchplaniq` CLI | [redacted] | Explain analyzer entrypoint |
| `searchplaniq-mcp` | [redacted] | MCP server for explain-plan tooling |
| `[redacted]` CLI | [redacted] | FTDC diagnostics and rule evaluation |
| `devprod-mcp-proxy` | [redacted] | Client-facing gateway proxy for MCP backends |
| `npm run electron:start` | [redacted] | Desktop diagnostic workstation start command |
...
| Salesforce | [redacted], [redacted], [redacted] | Source of support case data, auth flows, and customer metadata |
| Jira | [redacted], [redacted] | Issue and escalation context |
| Atlas API | [redacted], [redacted] | Used for FTDC requests and cluster diagnostics |
| FTDC | [redacted], [redacted] | Primary diagnostic artifact for storage/performance troubleshooting |
| Explain plans / Atlas Search | [redacted] | Central to search troubleshooting |
| MCP | [redacted], [redacted] | Tool aggregation and agent bridge pattern |
...
- For diagnostics, keep the extension lightweight: trigger actions through `[redacted]`, `[redacted]`, `[redacted]`, or `[redacted]` instead of rebuilding those engines in-browser.
- Use `[redacted]` when broad engineering-tool access is needed.
- Mine `[redacted]`, `[redacted]`, and `support-dashboard` for workflow ideas and UI patterns; avoid large-scale code copying unless dependencies are already aligned.
...
The generated script `install_[redacted]_case_tracker_dependencies.sh` is **clone-first** and **safe by default**:
- Clones high-signal repos into `~/Documents/GitHub/[redacted]` and installs prerequisite toolchains only when missing and when Homebrew is available.
...
| MCP | Model Context Protocol; tool-integration protocol used by `[redacted]` and `[redacted]` for agent-accessible tooling |
