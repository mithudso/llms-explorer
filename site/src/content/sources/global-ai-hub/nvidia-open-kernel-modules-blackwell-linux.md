---
title: "NVIDIA open kernel module on a Thunderbolt eGPU: Xid 79, driver blocking and power management"
description: "Running an RTX 50-series (Blackwell) GPU headless for CUDA on Ubuntu 26.04 — why Blackwell needs NVIDIA's open kernel modules and GSP firmware, the 580/595/610/615 driver branches, DKMS versus Canonic"
---

# NVIDIA open kernel module on a Thunderbolt eGPU: Xid 79, driver blocking and power management

Running an RTX 50-series (Blackwell) GPU headless for CUDA on Ubuntu 26.04 — why Blackwell needs NVIDIA's open kernel modules and GSP firmware, the 580/595/610/615 driver branches, DKMS versus Canonical-signed prebuilt modules, the module parameters that matter over a Thunderbolt tunnel, and the sm_120 status of CUDA, PyTorch, llama.cpp, vLLM and Ollama.
...
**Verified-as-of 2026-09-24.** Every claim is tagged `[SOURCED <url>]` or `[INFERRED]`. Parameter names
and version numbers are only stated where a primary source shows them; nothing is invented.

**Reference case (this box, 2026-09-24):** Ubuntu 26.04.1 · kernel `7.0.0-34-generic` · `nvidia-driver-610-open
610.57.04-0ubuntu0.26.04.3` via DKMS (builds on 7.0.0-27/31/34) · RTX 5080 (GB203) in a Thunderbolt 4 eGPU · Intel
Arrow Lake iGPU on `xe`/`i915` · `nvidia-smi` header reports `KMD 610.57.04 / CUDA UMD 13.3` · Ollama's
`llama-server` already runs on it · boot autoload blocked (`install nvidia /bin/false` in `/etc/modprobe.d`) and a
...
### 4. Driver branch taxonomy (NFB / PB / LTSB) and the 2026 landscape
...
### Compatibility matrix: driver ↔ CUDA toolkit ↔ sm_120
...
| Blackwell CCs | Consumer Blackwell = sm_120 (CC 12.0); data-center = sm_100/103; "SM101 was renumbered as SM110" in 13.0 | `[SOURCED 13.0/13.4 notes; PyTorch RFC]` |
...
### Ubuntu 26.04 (resolute) package inventory for branch 610 (2026-09-24)
...
### NVIDIA-repo alternative (do not mix with Ubuntu packages)
...
### How a kernel upgrade rebuilds the module (Ubuntu DKMS path)
1. `linux-image-generic-hwe-26.04` pulls `linux-headers-<new>`; DKMS needs "the `linux-headers` metapackage for your
...
5. Secure Boot: DKMS builds are not Canonical-signed; the DKMS package "may automatically guide you through creating
   and enrolling a new key" (MOK). If Secure Boot is on and MOK was never enrolled, the module will refuse to load
...
cp -a /etc/modprobe.d /var/backups/modprobe.d.$(date +%F)
...
#   -- and -- keep unattended-upgrades away:  Unattended-Upgrade::Package-Blacklist { "nvidia-"; "libnvidia-"; };

# 2. Kernel-only upgrade (same driver): let DKMS build, then verify before reboot
sudo apt upgrade
dkms status | grep 'nvidia/610.57.04' | grep "$(ls /lib/modules | sort -V | tail -1)"   # must say 'installed'
sudo reboot ; nvidia-smi

# 3. Driver point-release inside 610 (e.g. 610.57.04 -> 610.6x): unhold, upgrade, confirm firmware dir matches KMD
sudo apt-mark unhold '*nvidia*610*' && sudo apt upgrade && sudo reboot
ls /lib/firmware/nvidia/ ; nvidia-smi | head -5      # firmware dir == KMD version

# 4. Branch change (610 -> 615/620): only when a toolkit feature needs it (CUDA 13.4 needs R615)
sudo apt install nvidia-driver-615-open              # pulls 615 set, removes 610 set
#    then re-check: modprobe.d overrides still valid, Ollama/llama.cpp CUDA backend still detects GPU

# 5. Rollback (packages still in the pocket): pin to the exact version
V=610.57.04-0ubuntu0.26.04.3
sudo apt install nvidia-driver-610-open=$V nvidia-dkms-610-open=$V nvidia-kernel-source-610-open=$V \
...
|---|---|---|---|
| `NVreg_DynamicPowerManagement` (yes) | 0x00 / 0x01 / 0x02 / 0x03; default 3 | 0x00: "will disable runtime D3 power management features … the GPU … always is powered on." 0x01 coarse: lowest power state when no nvidia clients. 0x02 fine: additionally "actively monitor GPU usage while applications … are running." 0x03: "For Ampere or later notebooks with supported configurations, this value translates to fine-grained power control … For desktop computers, irrespective of the GPU(s) used, this value disables runtime D3." Requires Turing+, kernel ≥4.18, CONFIG_PM, "Intel Coffeelake or newer chipset". `[SOURCED https://download.nvidia.com/XFree86/Linux-x86_64/580.65.06/README/dynamicpowermanagement.html]` | **Keep 0x00.** On a *notebook host* the default (0x03) may resolve to fine-grained RTD3 for the eGPU-attached desktop GPU; an RTD3 cycle across a TB4 tunnel is exactly the "D3cold → D0" transition that fails in the field. `[INFERRED; failure quoted in issue #979]` |
| `NVreg_DynamicPowerManagementVideoMemoryThreshold` (yes) | MB, default 200, max 1024 | RTD3 only entered when used VRAM ≤ threshold; "A higher threshold value will increase the latency during RTD3 entry and exit". `[SOURCED same]` | Irrelevant once RTD3 is 0x00. |
| `NVreg_PreserveVideoMemoryAllocations` (yes) | 0 / 1 / 2(Auto); nv-reg.h default 2 | "0: Preserve only select video memory allocations 1: Preserve all video memory allocations 2: Auto". `=1` "changes the default video memory save/restore strategy to save and restore all video memory allocations" and "the `/proc/driver/nvidia/suspend` power management mechanism (with … systemd) is required"; companion `NVreg_TemporaryFilePath` (e.g. `/run`) must hold ≥ total VRAM + 5 %. `[SOURCED nv-reg.h; https://download.nvidia.com/XFree86/Linux-x86_64/580.65.06/README/powermanagement.html]` | `=0` (this box) is fine for a headless node that never suspends; if you do suspend, `=1` + `nvidia-suspend/hibernate/resume.service` + 16 GB+ free in `/run` or a disk path — but a TB4 eGPU rarely survives S3 anyway. `[INFERRED]` |
...
| `NVreg_EnableMSI` (yes) | 0 / 1; default 1 | enables PCI-E MSI. `[SOURCED nv-reg.h]` | Leave default. |
| `NVreg_OpenRmEnableUnsupportedGpus` (yes, deprecated) | default 1 | "This option to require opt in for use of Open RM on non-Data Center GPUs is deprecated and no longer required." `[SOURCED nv-reg.h]` | Remove from old configs. |
| `NVreg_UsePageAttributeTable` | **absent** from open-module `nv-reg.h` | — `[SOURCED nv-reg.h]` | Proprietary-era tuning; drop it. |
| `NVreg_RegistryDwords` (yes) | `key=value;…` | generic registry override. `[SOURCED nv-reg.h]` | Only under NVIDIA guidance. |

Related **kernel** command-line / udev knobs for a Thunderbolt eGPU (not NVreg):
...
Checklist for a box with no X/Wayland on the NVIDIA GPU (iGPU handles any console):
...
5. **Verify**: `nvidia-smi` (KMD == firmware dir == UMD-owning package version); `nvidia-smi -q -d POWER,PERFORMANCE`;
   `lspci -vv -s <bdf> | grep -E 'LnkSta|LnkCap'` expecting `16GT/s x4` over TB4. `[SOURCED link figure from issue #979; commands INFERRED]`
...
8. **Soak test before trusting**: `python3 -c "import torch; torch.zeros(1, device='cuda')"` is the minimal reproducer
   that hard-locked an RTX 5080 in a TB5 box on a TB4 host (issue #979); a long llama-server run is the realistic one.
   `[SOURCED issue #979]`
...
### Does apt's CUDA 12.4 userspace (`libcudart12 12.4.127`) matter?

| Consumer | Uses system CUDA libs? | Verdict for this box |
|---|---|---|
...
| **llama.cpp prebuilt** | No — release tarballs bundle per-CUDA builds plus a `cudart-…` runtime tarball | `llama-b11169-bin-ubuntu-cuda-12.8-x64.tar.gz` and `…-cuda-13.4-x64.tar.gz` + `cudart-llama-b11169-bin-ubuntu-cuda-12.8-x64.tar.gz` (and 13.4). `[SOURCED https://github.com/ggml-org/llama.cpp/releases]` Prefer the 12.8 or a ≤13.3 build on a 610 driver; a 13.4 build relies on minor-version compat and R615-gated calls could fail. `[INFERRED]` |
| **llama.cpp from source** | Yes — needs `nvcc` ≥12.8 to emit sm_120 | apt's 12.4 `nvcc` cannot target `compute_120`; install NVIDIA-repo `cuda-toolkit-12-8` or `cuda-toolkit-13-3` (never the bare `cuda` metapackage, which drags a driver). "By default llama.cpp will be built for the hardware that is connected" (or `-DCMAKE_CUDA_ARCHITECTURES=120`). `[SOURCED https://github.com/ggml-org/llama.cpp/blob/master/docs/build.md ; sm_120 floor SOURCED https://forums.developer.nvidia.com/t/software-migration-guide-for-nvidia-blackwell-rtx-gpus-a-guide-to-cuda-12-8-pytorch-tensorrt-and-llama-cpp/321330]` |
...
### LLM-stack status table (sm_120 / RTX 5080)

| Stack | sm_120 status (2026-09-24) | Notes / known issues |
|---|---|---|
| PyTorch | Supported since 2.7.0 (cu128); 2.11 cu130 Linux x86 lists Blackwell (10.0, 12.0). `[SOURCED https://journal.qualiteg.com/pytorch_and_supported_gpu_version/ ; https://github.com/pytorch/pytorch/issues/172663 via search]` | 2.12/2.13 matrix: CUDA 12.6 + 13.0 stable, 13.2 experimental, **12.8 removed** ("CUDA 12.8 is being deprecated and removed from CI/CD pipelines and binary build matrices", April 2026). `[SOURCED https://github.com/pytorch/pytorch/blob/main/RELEASE.md ; https://dev-discuss.pytorch.org/t/introducing-cuda-13-2-and-deprecating-cuda-12-8-release-2-12/3337]` Use `pip install torch` (cu130) on this box. |
...
| CUDA toolkit for dev | Match the UMD: `cuda-toolkit-13-3` (or 12.8) from NVIDIA repo. | 13.4 "Requires R615" for new features. `[SOURCED 13.4 release notes]` |
...
10. **Suspending the host with the eGPU attached and expecting VRAM to survive** — needs
    `PreserveVideoMemoryAllocations=1` + systemd suspend hooks + temp space ≥ VRAM; the box is at `=0`. `[SOURCED README powermanagement]`
