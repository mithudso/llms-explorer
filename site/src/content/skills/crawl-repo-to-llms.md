...
description: "Walks a repository and compiles the whole thing into an agent-loadable dossier: a per-file card, an importance ranking, condensed docs, the index inventory with embedding models and backends, the infrastructure map, and the git history."
order: 17
tags: [llms, repo, onboarding, inventory, context]
...
Two axes, both required. The **knowledge axis** is the documentation, condensed — that
...
importance tier assigned by a deterministic rubric rather than a vibe.
...
executed, not even the repo's own validator over the skill's own output. Secrets are
redacted and every redaction is reported. Edit guidance may only cite conventions the
repository actually states. Output never lands inside the target tree — including the case
where the target *is* the repository that contains the configured output directory.

Emits `llms.txt`, `llms-full.txt`, `llms-small.txt`, `llms-facts.txt`, plus
`llms-filemap.txt`, `llms-indexes.txt`, `llms-infra.txt`, `llms-history.txt`, and
machine-readable `filemap.json` + `manifest.json`. A self-check pass verifies the header
contract, the byte caps, filemap/manifest parity, and provenance coverage before anything
...
embedding model, how to query", "map the infra: CI, schedulers, env vars", incremental
`--refresh` runs that re-card only what changed since the last dossier's commit.
