...
`<REDACTED-{SLUG}>`, so a term like `acme-tool` became `<REDACTED-ACME-TOOL>` — invisible to a
case-sensitive `git log -S"acme-tool"`, found instantly by `-i`. That is not removal. Use the
