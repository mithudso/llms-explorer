---
description: Customer → LLMS — crawl a customer engagement folder on the [redacted] shared drive, harvest that customer's context/artifact files and case, ticket and channel records from Glean, merge both halves into one deduped provenance-tagged truth pack, and write the llms.txt family into that customer's own folder
argument-hint: customer name or alias ([redacted], [redacted], Apple…) [--depth quick|standard|deep|--since YYYY-MM-DD|--stale-after DAYS|--scope SUBPATH|--no-glean|--glean-only|--include-archive|--refresh|--force|--yes]
---
