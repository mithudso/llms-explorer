...
2. **Acquire.** Prefer the site's own llms files; fall back to a crawl. Both write one
   ...
   ```
   .venv/bin/python scripts/text_mirror.py https://code.claude.com/docs --prefer-llms --out mirrors/code.claude.com.md
   ```
...
6. **Query.** All three modes, no network.
   ...
   ```
   .venv/bin/python scripts/docset_indexer.py keyword code.claude.com "X-Markdown-Tokens" --mode phrase
   .venv/bin/python scripts/docset_indexer.py query   code.claude.com "how big is a page before I fetch it" --layer facts
...
pages, an index of ~280 tokens, small at ~49.8k, facts with 14,031 units). After step 4,
`docset_indexer.py list` shows `code.claude.com` and `code.claude.com__facts` with their
counts. After step 5:
...
and the keyword query prints the `X-Markdown-Tokens` unit with its `url#anchor`, exactly
as the hosted tool would.
