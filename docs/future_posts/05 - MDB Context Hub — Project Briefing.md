...
| **Manual lookups** — which [redacted] repo, shared module, or MCP server applies | `tam_recommend_repo_libraries` / `tam_recommend_shared_libraries` / `tam_recommend_mcps` return scored recommendations |
