...
- Bug found — `gen_concepts.py` is not safe to run bare in this repo: it fully regenerates every `site/src/data/concepts/*.json` from `~/.global-ai-hub/llms-concepts/`, but 7 committed concept files (`agents-md-ucp`, `cloudflare-ai-crawler-monetization-verification`, `eu-ai-act-tdm-opt-out`, `llms-txt`, `nlweb-mcp-agentic-discovery`, `really-simple-licensing`, `robots-txt-content-signals`) carry a "Where this helps / How to apply this" facet layer added by some other (unidentified) enrichment pass that isn't part of the base `.llms` pack format the script reads. Running it blindly silently deletes that layer from every existing concept it doesn't have hub-dir source for. Caught via `git status` before commit, restored the 7 files with `git checkout`, confirmed via parsed-JSON diff (not text diff) that zero data was lost. **The script itself needs a fix**: either a `--only <slug>` scoping flag, or the enrichment pass needs to run as a documented post-step after every `gen_concepts.py` run rather than being a silent one-off. Whoever finds the enrichment pass should wire it into the same `postbuild`/generator chain so this can't recur.
- Decision: built the concept pack with the actual `llms-concept-abstractor` tool (lexicon → harvest → classify → compile) against two real sources — `~/.claude/skills/integration-clients/references/glean-dev.md` (updated in the same pass, v1.1.0 to v1.2.0) and a hands-on rabbithole dossier (`~/glean-cli-dossier.md`, live binary + live tenant API calls) — rather than hand-writing JSON. Kept 2 genuine disagreements (credential-resolution model vs. observed behavior; `--dry-run` doc claim vs. verified behavior) side by side under `## Disagreements`, not merged.
- Completed: promoted `glean-enterprise-cli` from frontier to a real node in `concept-tree/tree.json` (509 nodes, 3063 frontier, down from 3064), regenerated `site/src/data/tree.json` and the one new `site/src/data/concepts/glean-enterprise-cli.json`, indexed the pack (`concept__glean-enterprise-cli`, vector + FTS5, 69 units, 0 failures).
- Verification: `npm run build` (1050 pages, no errors); `uv run --directory hub pytest ../site/tests/test_gen_tree.py ../site/tests/test_gen_concepts.py ../site/tests/test_tree_pages.py ../hub/tests/test_concept_tree.py` — 46 passed (2 initially failed pre-build, expected — they check the built static page exists; passed after `npm run build`). Confirmed live retrieval via `hub_query_docset(concept__glean-enterprise-cli, "dry-run vs preview", mode=hybrid)` — correct passage at rank 1.
- Changed files: `concept-tree/tree.json` (+15 lines, one node), `site/src/data/tree.json` (regenerated), `site/src/data/concepts/glean-enterprise-cli.json` (new). The 7 unrelated concept files are untouched in the final commit.
- Remaining: the `gen_concepts.py` enrichment-stripping gap above is unresolved — next person to add or refresh a concept pack should check `git diff` on all concept JSONs before committing, not just the one they intended to touch, until the script or the enrichment pass is fixed.
...
- **Status**: Done and verified. `origin/main` is at `c4c985f`. Postgres on this machine (`m5`, Tailscale IP `198.51.100.117`) is the shared dev database, reachable over the tailnet.
- **Completed work**:
...
- **Next steps**: teammates who want to use this shared Postgres from another Tailscale device need `DATABASE_URL=postgresql+asyncpg://explorer:<password>@198.51.100.117:5432/explorer` in their own `api/.env`, plus the same `OAUTH_GITHUB_ID/SECRET` and `OAUTH_GOOGLE_ID/SECRET` — user distributes these out-of-band, never via git; the pre-existing `test_plans.py` seed-drift failure is still open, unrelated to auth/OAuth work.
